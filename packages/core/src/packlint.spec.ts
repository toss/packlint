...
function toIssueMeta(issue: { message: string; fixable: boolean; fixed: boolean }): {
  message: string;
  fixable: boolean;
  fixed: boolean;
} {
  return { message: issue.message, fixable: issue.fixable, fixed: issue.fixed };
}
...
describe('packlint', () => {
  it('applies fixes in plugin order and reports each issue', async () => {
    ...
    expect(diagnostic.issues.map(toIssueMeta)).toEqual([
  ...
  it('supports async check and async fix', async () => {
    ...
    expect(diagnostic.issues.map(toIssueMeta)).toEqual([{ message: 'set type', fixable: true, fixed: true }]);
  });
